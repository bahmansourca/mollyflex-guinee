...
export async function GET() {
  ...
  const sales = await prisma.sale.findMany({
    ...
    select: { createdAt: true, totalAmount: true, workerId: true, productId: true, isLoan: true, paymentStatus: true },
  });
  ...
  const products = await prisma.product.findMany({ where: { id: { in: Array.from(byProduct.keys()) } }, select: { id: true, name: true } });
  ...
  const seriesProduct = products.map(p => ({ label: p.name, value: byProduct.get(p.id) || 0 }));
